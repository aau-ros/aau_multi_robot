...
robot_state::robot_state_t RobotStateApi::getRobotStateEnum() { //TODO raise exception //TODO put locks
    robot_state::GetRobotState get_srv_msg;
    bool call_succeeded = get_robot_state_sc.call(get_srv_msg);
    while(!call_succeeded) { //TODO max number of retries
        ROS_ERROR("call failed");   
    ...
    return static_cast<robot_state::robot_state_t>(get_srv_msg.response.robot_state); //TODO safety check to avoid out_of_range
}
...
RobotState* RobotStateApi::enumToState(unsigned int enum_val) {
    return enumToStateMap.at(enum_val);
}

unsigned int RobotStateApi::stateToEnum(RobotState *state) {
    return stateToEnumMap.at(state);
}
