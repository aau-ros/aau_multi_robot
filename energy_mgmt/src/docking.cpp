...
//    target_ds->id = -1;
    ...
    */
    ...
    goal_ds_for_path_navigation_sub = nh.subscribe("energy_mgmt/graph_path_topic", 100, &docking::simple_compute_and_publish_path_on_ds_graph_topic, this);
...
//    target_ds_set = false;
    ...
    sub_finalize_exploration = nh.subscribe("finished_exploration", 10 , &docking::finalize_exploration_callback, this);
...
//        set_optimal_ds_vacant(true);
    ...
    {
        if(!already_sent_vacancy_check_request) {
            already_sent_vacancy_check_request = true;
...
//                    start_new_auction();
    ...
    else if (next_robot_state == CHARGING_COMPLETED)
    {
        ;
    }
    else if (next_robot_state == CHARGING_ABORTED)
    {
        ;
    }
    else if (next_robot_state == auctioning_3)
    ...
    else if (next_robot_state == CHOOSING_ACTION)
    {
        ;
    }
    else
    ...
    if (next_robot_state != robot_state::CHECKING_VACANCY) {
        ROS_DEBUG("setting already_sent_vacancy_check_request to false");
        already_sent_vacancy_check_request = false;
    }
                                                                                                  ...
                                                                                                  // choices
{
    ...
    if (optimal_ds_is_set() && msg.get()->id == get_optimal_ds_id())
        ...
        if (using_ds(robot_state) || using_ds(next_robot_state))
        {
            adhoc_communication::SendEmDockingStation srv_msg;
...
void docking::inform_explorer_about_used_ds() {
...
void docking::simple_compute_and_publish_path_on_ds_graph_topic(const explorer::RequestPath::ConstPtr &msg) {

    ROS_INFO("simple computing path on DS graph with fallback procedure (publisher-subscribe)");
    moving_along_path = true;
    ...
    mutex_ds_graph.lock();
    bool ds_found_with_mst = false;
    if(msg.get()->goal_ds_id == msg.get()->closest_ds_id) {
        log_minor_error("closest_ds->id == goal_ds_path_id, this should not happen...");
        ds_found_with_mst = true;
        path_navigation_tries++;
        path.push_back(msg.get()->goal_ds_id);
    }
    else {
        ds_found_with_mst = find_path_2(msg.get()->closest_ds_id, msg.get()->goal_ds_id, path);
        path_navigation_tries = 0;    
    }
    mutex_ds_graph.unlock();
...
//            printed_stuck = false;
        ...
        check_reachable_ds();   
        compute_optimal_ds();
        if(ros::Time::now() - last_sent > ros::Duration(2)) {
            send_ds();
            send_optimal_ds();
