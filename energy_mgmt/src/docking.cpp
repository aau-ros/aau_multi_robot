...
docking::docking()  // TODO(minor) comments //TODO(minor) comments in .h file
{
    ...
    nh_tilde.param<float>("safety_coeff", safety_coeff, 1); //not used at the moment

    
    ROS_ERROR("%d, %d, %d", ds_selection_policy, auction_timeout, reauctioning_timeout);
...
void docking::compute_optimal_ds()
{
    ...
    if (ds.size() > 0) //TODO CHECKS ALL POLICIES
    {
        ...
        /* NB: even if the robot position does not change during the execution of this function, the distance between the robot and a fixed point computed using distance_from_robot() could change, because it calls a function in explorer node that uses a costmap to compute the distance, and since the costmap could be updated during the execution of compute_optimal_ds() (or, more simply, the algorithm used to compute the distance is not 100% precise), the distance could change: this means that we need to check at the end of the computation of the new optimal DS if it is really a different DS from the previous one; this can happens for instance because the other robots are moving. Of course the differences between two different calls of distance_from_robot with the same point as argument will be very little, but they are enough to make us think that we have found a DS that is better than the one already selected, even if the new DS is exactly the old one. Another problems is that we could select a DS that is not really the closest at the end of the execution of compute_optimal_ds() because the distances could have changed, but we accept this */
            ...
            if (!moving_along_path) {
                ...
                if (!found_reachable_ds_with_eo) //TODO check this part (in particular for the moving_along_path)
                {
                    if (found_ds_with_eo)
                    {
                                        ...
                                        if (dist < battery.remaining_distance / 2)
                                        {
                                            ...
                                            for (int k = 0; k < ds.size(); k++) {
                                                ...
                                                if(dist < 0)
                                                    continue;
                                                if (k != i &&
                                                    dist < battery.remaining_distance / 2) {
                        ...
                        compute_closest_ds(); //TODO although probably the robot will think that the exploration is over given how explorer works for the moment...
                }
            }
            else
        ...
        if (old_optimal_ds == NULL || old_optimal_ds->id != best_ds->id)
        {
            if(old_optimal_ds != NULL)
                ...
                fs_csv << time.toSec() << "," << best_ds->id << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
                fs_csv.close();
...
void docking::cb_robots(const adhoc_communication::EmRobot::ConstPtr &msg)
{
    ...
    fs3_csv << time.toSec() << "," << msg.get()->id << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
    fs3_csv.close();
...
void docking::cb_auction_reply(const adhoc_communication::EmAuction::ConstPtr &msg)  // TODO check
{
    ...
    if(auction_id != msg.get()->auction) {
        ROS_INFO("The received bid is not for the auction recently started by this robot: ignore it");
        return;
    }
    
    if (!managing_auction) {
        ROS_INFO("The received bid has arrived too late, since the associated auction has already finished: ignore it");
        return;    
    }
...
{
    started_own_auction = true; //otherwise a robot could not start the auction because the following if is true, then win another robot auction and stop the time, then lost another and not be reset in queue...
    ...
    if(participating_to_auction == 0) //Notice that it is still possible that two robots start an auction at the same time...
        ...
        ROS_ERROR("Robot is already participating to an auction: let's wait instead of starting another one...");
        timer_restart_auction.setPeriod(ros::Duration(reauctioning_timeout), true);
        timer_restart_auction.start();
    }
}
...
{
    fs_csv.open(csv_file.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs_csv << "#time,optimal_ds,num_robots,ds_selection_policy,start_absolute_x,starting_absolute_y" << std::endl;
    fs_csv.close();
    
    fs2_csv.open(csv_file_2.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs2_csv << "#time,x,y,num_robots,ds_selection_policy,start_absolute_x,starting_absolute_y" << std::endl;
    fs2_csv.close();
    
    fs3_csv.open(csv_file_3.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs3_csv << "#time,sender_robot_id,num_robots,ds_selection_policy,start_absolute_x,starting_absolute_y" << std::endl;
    fs3_csv.close();
    
    fs_info.open(info_file.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs_info << "#robot_id,num_robots,ds_selection_policy,start_absolute_x,starting_absolute_y" << std::endl;
    fs_info << robot_id << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
    fs_info.close();
...
void docking::send_robot() {
    ...
    ros::Duration time = ros::Time::now() - time_start;
    fs2_csv.open(csv_file_2.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs2_csv << time.toSec() << "," << robot.x << "," << robot.y << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
    fs2_csv.close();
...
void docking::finalize() {
    ...
    fs_csv.open(csv_file.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs_csv << time.toSec() << "," << "-1" << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
    fs_csv << "#end";
    fs_csv.close();
    
    fs3_csv.open(csv_file_3.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    fs3_csv << time.toSec() << "," << "-1" << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl;
    fs3_csv << "#end";
    fs3_csv.close();
    
    fs2_csv.open(csv_file_2.c_str(), std::fstream::in | std::fstream::app | std::fstream::out);
    //fs2_csv << time.toSec() << "," << origin_absolute_x << "," << origin_absolute_x << "," << num_robots << "," << ds_selection_policy << "," << origin_absolute_x << "," << origin_absolute_y << std::endl; //TODO do better...
    fs2_csv << "#end";
    fs2_csv.close();
