...
docking::docking()
{
    ...
    {
        /* Empty_prefix: hardware platform (real experiment) */
        char hostname[1024];
        hostname[1023] = '\0';
    ...
    sub_ds_state_update =
        nh.subscribe("adhoc_communication/ds_state_update", 1, &docking::ds_state_update_callback, this);
    ...
    pub_new_target_ds = nh.advertise<geometry_msgs::PointStamped>("new_target_docking_station_selected", 1);
    ...
    pub_auction_result = nh.advertise<std_msgs::Empty>("explorer/auction_result", 1);
...
void docking::preload_docking_stations()
{
    ...
    /* Set the initial optimal DS to ds0: this is just to initialize the variable, so that the can compute the distance
     * from the robot and the currently selected DS and compare it with the distance from the other DSs also when the
     * node starts */
...
void docking::check_vacancy_callback(const std_msgs::Empty::ConstPtr &msg) //TODO
{
...
void docking::update_robot_state() //TODO simplify here...
{
    /*
     * Check if:
