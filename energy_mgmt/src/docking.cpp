...
docking::docking()  // TODO(minor) create functions; comments here and in .h file; check if the topics uses the correct messages...
{
    ...
    nh_tilde.param<float>("safety_coeff", safety_coeff, 0.8);                     // not used at the moment //TODO(IMPORTANT) coeff
    ...
    */
    ...
    sub_resend_ds_list = nh.subscribe("adhoc_communication/resend_ds_list", 100, &docking::resend_ds_list_callback, this);
    sub_full_battery_info = nh.subscribe("full_battery_info", 100, &docking::full_battery_info_callback, this);
    ...
    if (ds.size() > 0)
    {
        ...
        else if (old_optimal_ds == NULL || old_optimal_ds->id != best_ds->id)
        {
            if (old_optimal_ds != NULL)
                ...
                ROS_INFO("Select optimal DS: ds%d", best_ds->id);
...
void docking::update_l3()
{
    ...
    {
        ++num_jobs;
        double dist = distance_from_robot(jobs[i].x, jobs[i].y, true); // use euclidean distance to make it faster //TODO(minor) sure? do the same somewhere else?
...
double docking::distance(double start_x, double start_y, double goal_x, double goal_y, bool euclidean)
{
    ...
    /* If the service is not working at the moment, return invalid value */
...
void docking::cb_robot(const adhoc_communication::EmRobot::ConstPtr &msg)  // TODO(minor) better name and do better
{
    ...
    else if (msg.get()->state == checking_vacancy)
    {
        ...
        srv_msg.request.dst_robot = "mc_robot_0";
...
void docking::cb_new_auction(const adhoc_communication::EmAuction::ConstPtr &msg)
{
    ...
    if(!already_known_ds) {
        ...
        srv_msg.request.dst_robot = "mc_robot_0"; //TODO(minor) use a string everywhere...
        sc_send_docking_station.call(srv_msg);
        return;
    }
    ...
    if (index_of_ds_in_path < path.size() - 1)
    {
        ...
        for (int i = 0; i < ds.size(); i++)
            ...
            {   
                ...
                best_ds = &ds[i];    // TODO(IMPORTANT) VERY BAD!!!!
                break;
            }
    }
    else {
...
void docking::resend_ds_list_callback(const adhoc_communication::EmDockingStation::ConstPtr &msg) { //TODO(minor) do better...
    ROS_ERROR("Sending complete list of discovered docking stations");
    for(std::vector<ds_t>::iterator it = discovered_ds.begin(); it != discovered_ds.end(); it++) {
        adhoc_communication::SendEmDockingStation srv_msg;
        srv_msg.request.topic = "docking_stations";
