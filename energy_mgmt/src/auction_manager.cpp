...
//        }
    ...
    if(auction_participation_state == PARTICIPATING)
        ...
        ROS_INFO("Starting new auction (%d)", current_auction.auction_id); //TODO auction id
    ...
    if(optimal_ds_is_set && (unsigned int)msg.get()->docking_station == optimal_ds_id) {
        if(auction_participation_state == MANAGING) {
            ...
            if(current_auction.starting_time < msg.get()->starting_time) {
                ...
                current_auction.auctioneer = msg.get()->robot;
                current_auction.auction_id = msg.get()->auction;
                current_auction.docking_station_id = msg.get()->docking_station;
                current_auction.starting_time = msg.get()->starting_time;
