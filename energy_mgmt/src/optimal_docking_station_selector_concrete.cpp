...
OptimalDockingStationSelectorConcrete::OptimalDockingStationSelectorConcrete() {
...
void OptimalDockingStationSelectorConcrete::computeOptimalDs() //TODO(minor) best waw to handle errors in distance computation??
...
//        jobs_mutex.unlock();
    ...
    else if (docking_station_selection_policy == 1)
        vacantPolicy();

    else if (docking_station_selection_policy == 2)
        opportuneOptimalDs();
...
//DONE++
void OptimalDockingStationSelectorConcrete::rel_to_abs(double relative_x, double relative_y, double *absolute_x, double *absolute_y)
{
    ROS_FATAL("MISSING");
    // Use these if the /map frame origin coincides with the robot starting position in Stage (which should be the case)
...
void OptimalDockingStationSelectorConcrete::opportuneOptimalDs() {
...
void OptimalDockingStationSelectorConcrete::currentOptimalDs()
{
...
void OptimalDockingStationSelectorConcrete::flockingOptimalDs() {
...
//                count++;
//        double n_r = (double)count / (double)robots.size();
...
//            sum_y += robots.at(i).y;
//        }
//        double flock_x = (double)sum_x / (double)robots.size();
//        double flock_y = (double)sum_y / (double)robots.size();
