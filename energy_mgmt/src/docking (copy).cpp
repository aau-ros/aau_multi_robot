...
void docking::compute_optimal_ds()
{
   ...
   if(computed_new_optimal_ds)
       if(participating_to_auction == 0) {
            ROS_ERROR("\e[1;34mNew optimal DS: ds%d (%f, %f)\e[0m", next_optimal_ds.id, next_optimal_ds.x, next_optimal_ds.y);
            ...
            ROS_ERROR("\e[1;34mNext optimal DS: ds%d (%f, %f)\e[0m", next_optimal_ds.id, next_optimal_ds.x, next_optimal_ds.y);
     else
...
void docking::cb_auction(const adhoc_communication::EmAuction::ConstPtr &msg)
{
    ...
    */
    ...
    {
        /* Robot received a bid of an auction whose auctioned docking station is not the one the robot is interested in at the moment, so it won't participate to the auction */
