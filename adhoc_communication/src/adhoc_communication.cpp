...
void processCrSelectionFrame(CrSelectionFrame* f)
{
    ...
    ROS_INFO("GOT CR SELECTION FROM %s FOR %s RELAY INDEX[%u]", getMacAsStr(f->eh_h_.eh_source).c_str(), getMacAsStr(f->header_.relay_connection).c_str(), f->header_.relay_index);
...
void processRouteResponse(RouteResponse rr)
{
    ...
    if (find_rreq != route_requests_l.end())
    {
        ...
        if (compareMac(rr.mac_current_hop_, src_mac) && request.response_sent == false)
        {
            ...
            ROS_INFO("I AM NOT THE CURRENT HOP[%s] TO FORWARD THE RESPONSE!", getMacAsStr(rr.mac_current_hop_).c_str());
        ...
        if (enable_cooperative_relaying)// && rr.mc_flag_ == false)todo
        {
            ...
            if (crRouteAvailable(&rr, cr_route))
            {
                ...
                if (find_cr_route != cr_entries_l.end())
                {
                    ...
                    {
                        ROS_INFO("COULD ACT AS RELAY BETWEEN %s and %s", (getMacAsStr(cr_route.first_mac)).c_str(), (getMacAsStr(cr_route.second_mac)).c_str());
...
void addGroupToRelay(unsigned char* mac1, unsigned char* mac2, std::string group_name)
{
    ...
    if (find_relay != cr_entries_l.end())
    {
        if ((*find_relay).addMcRelayConnection(mac1, mac2, group_name))
            ROS_INFO("ADD MC ENTRY FOR RELAY CONNECTION FROM %s TO %s GROUP[%s]", getMacAsStr(mac1).c_str(), getMacAsStr(mac2).c_str(), group_name.c_str());

        if ((*find_relay).addMcRelayConnection(mac2, mac1, group_name))
            ROS_INFO("ADD MC ENTRY FOR RELAY CONNECTION FROM %s TO %s GROUP[%s]", getMacAsStr(mac2).c_str(), getMacAsStr(mac1).c_str(), group_name.c_str());
