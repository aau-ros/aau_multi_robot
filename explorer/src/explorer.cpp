        ...
        while (robot_state != finished)
        {
            ...
            */
            ...
            if (robot_state == exploring || robot_state == fully_charged)
            {
                ...
                */
                ...
                else if (frontier_selection == 7)
                {
                    ...
                    /* If the robot is fully charged, it means that it will never be able
                       to reach any new frontier even after a recharging: exploration is
                       over */
            ...
            if (robot_state == going_in_queue || robot_state == going_checking_vacancy || robot_state == going_charging)
            {
                ...
                /*
                int distance_temp = exploration->trajectory_plan(home_point_x - 2,
                home_point_y - 2);
                int x_temp = home_point_x - 2;
                int y_temp = home_point_y - 2;
                if(distance_temp > exploration->trajectory_plan(home_point_x - 2,
                home_point_y + 2)) {
                    distance_temp = exploration->trajectory_plan(home_point_x - 2,
                home_point_y + 2);
                    x_temp = home_point_x - 2;
                    y_temp = home_point_y + 2;
                } else if (distance_temp > exploration->trajectory_plan(home_point_x +
                2, home_point_y + 2)) {
                    distance_temp = exploration->trajectory_plan(home_point_x + 2,
                home_point_y + 2);
                    x_temp = home_point_x + 2;
                ...
                /*
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x - 2,
                home_point_y - 2));
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x - 2,
                home_point_y    ));
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x - 2,
                home_point_y - 2));
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x    ,
                home_point_y - 2));
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x    ,
                home_point_y    ));
                ROS_ERROR("%d", exploration->trajectory_plan(home_point_x    ,
