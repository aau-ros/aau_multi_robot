  ...
  public:
    ...
    void explore()  // TODO(minor) comments
    {
        ...
        while (robot_state != finished)
        {
            ...
            */
                        ...
                        if(OPP_ONLY_TWO_DS)
                            ...
                            if(ds_path_counter < ds_path_size - 1)
                            {
                                ...
                                for(int i=0; i<10 || dist >= 0; i++) {
                                    dist = exploration->distance_from_robot(next_ds_x, next_ds_y); //TODO(IMPORTANT) very bad way to check... -> parameter...
                                    ros::Duration(1).sleep();   
                                }
                                
                                if(dist > available_distance * 0.8) //TODO(IMPORTANT) 0.8 -> parameter, and in determine_goal, etc....
            ...
            {
                // TODO(IMPORTANT) what is this part???
            ...
            if (robot_state == auctioning || robot_state == auctioning_2 )
            {
                ...
                if (exploration->distance_from_robot(target_ds_x, target_ds_y) <
                        queue_distance / 2.0)  // TODO could the DS change meanwhile??? //TODO(IMPORTANT) do better
                        {
                            ROS_ERROR("ROBOT TOO CLOSE TO DS to start an auction: moving a little bit farther..."); 
            ...
            if (robot_state == in_queue)
            {
                while (robot_state == in_queue)  // TODO(minor) better management of the while loop
                {
                    ...
                    if (exploration->distance_from_robot(target_ds_x, target_ds_y) <
                        queue_distance / 2.0) //TODO(IMPORTANT) do better
            ...
            if (robot_state == checking_vacancy)
            {
                ...
                // TODO(IMPORTANT) use a bterr way!!!
                int i = 10;
                while (i > 0)
                {
                    ros::Duration(1).sleep();
                ...
                if (occupied_ds)
                {
                    // TODO(IMPORTANT) opportune? no because if I ahve this DS in the path, it's because I need to recharge here
    ...
    void bat_callback(const energy_mgmt::battery_state::ConstPtr &msg)
    {
        ...
        if (msg->charging == false && battery_charge == 100 && charge_time == 0)
            recharge_cycles++;  // TODO(IMPORTANT) hmm... soc, charge, ...
