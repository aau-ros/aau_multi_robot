...
double ExplorationPlanner::trajectory_plan_meters(double target_x, double target_y)
{
    ...
    //ROS_ERROR("%f", costmap_global_ros_->getCostmap()->getResolution());
    return trajectory_plan_meters(robotPose.getOrigin().getX(), robotPose.getOrigin().getY(), target_x, target_y);
}
...
void ExplorationPlanner::frontierCallback(const adhoc_communication::ExpFrontier::ConstPtr& msg)
{
    ...
    //ROS_ERROR("frontierCallback, %lu", msg.get()->frontier_element.size());
...
bool ExplorationPlanner::existFrontiersReachableWithFullBattery(float max_available_distance) {
    ...
    for (int i = 0; i < frontiers.size(); i++)
    {
        if (check_efficiency_of_goal(frontiers.at(i).x_coordinate, frontiers.at(i).y_coordinate) == true)
        {
            ...
            distance = trajectory_plan_meters(frontiers.at(i).x_coordinate, frontiers.at(i).y_coordinate, robot_home_x, robot_home_y);
            if(distance < 0){
                ROS_ERROR("Failed to compute distance!");
                continue;
            }
...
double ExplorationPlanner::distance_from_robot(double x, double y) {
    ...
    return trajectory_plan_meters(x, y);
}
...
double ExplorationPlanner::distance(double x1, double y1, double x2, double y2) {
    ...
    return trajectory_plan_meters(x1, y1, x2, y2); //return a (very very rough) distance in meters
}
...
//    }
    ...
    for(int i=0; i < frontiers.size(); i++) {
        ...
        if(index_optimal_ds >= ds_graph.size() || index_closest_ds_to_robot >= ds_graph[index_optimal_ds].size() || index_closest_ds_to_robot < 0 || index_optimal_ds < 0 || index_closest_ds_to_frontier < 0)
            ROS_ERROR("invalid index(-ces)"); //: %d, %d, %lu", index_optimal_ds, index_closest_ds_to_robot, ds_graph.size());
            //ROS_ERROR("index_optimal_ds is too high; ds_graph size: %lu; index_optimal_ds: %d", ds_graph.size(), index_optimal_ds);
...
void ExplorationPlanner::acquire_mutex(boost::mutex *mutex, std::string function_name) {
    ROS_DEBUG_NAMED("locks", "Function '%s' is trying to acquire mutex...", function_name.c_str());
    mutex->lock();
    ROS_DEBUG_NAMED("locks", "%s acquired by function '%s'", "Mutex", function_name.c_str());
}
...
void ExplorationPlanner::release_mutex(boost::mutex *mutex, std::string function_name) {
    mutex->unlock();
    ROS_DEBUG_NAMED("locks", "%s released by function '%s'", "Mutex", function_name.c_str());
}
